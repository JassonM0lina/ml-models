"""
Saudi Arabia Crude Oil Burn Forecaster - Training Script
SageMaker-compliant training pipeline for XGBoost ensemble model

This script:
1. Loads historical burn data from S3 (via SageMaker input channel)
2. Fetches weather data from Open-Meteo API for major Saudi cities
3. Engineers features including temperature extremes, cooling degree days, and lag variables
4. Trains XGBoost model with summer-weighted samples
5. Evaluates model performance and saves artifacts
"""
...
def upload_evaluation_to_s3(evaluation_data: dict, eval_s3_path: str) -> str:
    ...
    Args:
        ...
        eval_s3_path: S3 URI of evaluation_data folder
...
def parse_args():
    ...
    parser.add_argument("--summer-weight", type=float, default=5.0)
    parser.add_argument("--peak-weight", type=float, default=10.0)
...
# =============================================================================
# STEP 1: DATA LOADING AND WEATHER FETCHING
# =============================================================================

def load_burn_data(input_path):
    """Load historical burn data from SageMaker input channel"""
    print("=" * 60)
    ...
    if burn_df.shape[1] == 2:
        ...
        date_col = [col for col in burn_df.columns if 'date' in col.lower()]
        burn_col = [col for col in burn_df.columns if 'burn' in col.lower() or 'saudi' in col.lower()]
        if date_col and burn_col:
            burn_df = burn_df[[date_col[0], burn_col[0]]]
            burn_df.columns = ['date', 'burn_bpd']
...
def fetch_historical_weather(burn_df):
    ...
    print(f"  Total daily weather records: {len(daily_weather)}")
    return daily_weather


def aggregate_monthly_weather(daily_weather):
    """Aggregate daily weather to monthly features"""
    print("\n" + "=" * 60)
    print("STEP 3: MONTHLY WEATHER AGGREGATION")
    print("=" * 60)

    weather_monthly = daily_weather.groupby(['year', 'month']).agg({
    ...
    print(f"  Created {len(weather_monthly.columns)} monthly weather features")
    return weather_monthly


def prepare_features(burn_df, weather_monthly):
    """Merge data and create all model features"""
    print("\n" + "=" * 60)
    ...
    # Ramadan summer indicator (approximate years when Ramadan fell in summer)
    ...
    # Gas availability proxy (decreasing over time)
    ...
    # Summer-specific interaction features
...
def train_model(merged_df, args):
    """Train XGBoost model with summer-weighted samples"""
    print("\n" + "=" * 60)
    ...
    split_idx = len(X) - args.test_months
    X_train, X_test = X[:split_idx], X[split_idx:]
    y_train, y_test = y[:split_idx], y[split_idx:]
    is_summer_train = is_summer[:split_idx]
    is_peak_train = is_peak[:split_idx]
    is_summer_test = is_summer[split_idx:]
    is_peak_test = is_peak[split_idx:]

    print(f"  Train samples: {len(X_train)}, Test samples: {len(X_test)}")

    # Create sample weights (emphasize summer months)
    sample_weights = np.ones(len(y_train))
    sample_weights[is_summer_train == 1] = args.summer_weight
    sample_weights[is_peak_train == 1] = args.peak_weight
    ...
    test_data = {
        ...
        'test_dates': merged_df.iloc[split_idx:]['date'].values
    }

    print(f"  Model trained successfully")
    return model, feature_cols, test_data
...
def evaluate_model(model, feature_cols, test_data):
    ...
    # Summer metrics
    summer_mask = is_summer_test == 1
    if summer_mask.sum() > 0:
        summer_mae = mean_absolute_error(y_test[summer_mask], y_pred[summer_mask])
        summer_mape = mean_absolute_percentage_error(y_test[summer_mask], y_pred[summer_mask]) * 100
    else:
        summer_mae = 0
        summer_mape = 0

    # Peak summer metrics
    peak_mask = is_peak_test == 1
    if peak_mask.sum() > 0:
    ...
    print(f"  Peak (Jul-Aug) MAE: {peak_mae:.1f} kbd")
    ...
    evaluation = {
        ...
        "summer_mae": round(summer_mae, 2),
        "summer_mape": round(summer_mape, 2),
        "peak_mae": round(peak_mae, 2),
...
def save_outputs(model, feature_cols, evaluation, args):
    ...
    # Create model package (compatible with existing inference code)
    model_package = {
        'model': model,
        'model_name': 'XGBoost_SaudiBurn',
    ...
    # Copy inference.py to code directory
    ...
    requirements_root_dst = os.path.join(args.model_dir, "requirements.txt")
...
'''
    for path in [os.path.join(code_dir, "setup.py"), os.path.join(args.model_dir, "setup.py")]:
    ...
    # Save metadata
    metadata = {
        "model_name": "Saudi Oil Burn Forecaster",
        "model_type": "XGBoost",
        "training_date": datetime.now().isoformat(),
...
def main():
    ...
    print("\n" + "=" * 60)
    print("SAUDI OIL BURN FORECASTER - TRAINING PIPELINE")
    print("=" * 60)
    print(f"Training Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 60)

    # Step 1: Load burn data
    burn_df = load_burn_data(args.train)

    # Step 2: Fetch weather data
    daily_weather = fetch_historical_weather(burn_df)

    # Step 3: Aggregate to monthly
    weather_monthly = aggregate_monthly_weather(daily_weather)

    # Step 4: Prepare features
    merged_df = prepare_features(burn_df, weather_monthly)

    # Step 5: Train model
    model, feature_cols, test_data = train_model(merged_df, args)

    # Step 6: Evaluate model
    evaluation = evaluate_model(model, feature_cols, test_data)

    # Save outputs
    save_outputs(model, feature_cols, evaluation, args)

    print("\n" + "=" * 60)
    print("TRAINING PIPELINE COMPLETE")
    print("=" * 60)
    print(f"  Model saved to: {args.model_dir}")
    print(f"  Overall MAE: {evaluation['overall_mae']} kbd")
    print(f"  Summer MAE: {evaluation['summer_mae']} kbd")
    print("=" * 60)
